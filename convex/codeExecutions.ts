...
export const getUserStats = query({
  ...
  handler: async (ctx, args) => {
    const executions = await ctx.db
      ...
      .filter((q) => q.eq(q.field('userId'), args.userId))
      .collect()
    const starrtedSnippets = await ctx.db
      .query('stars')
      .withIndex('by_user_id')
      .filter((q) => q.eq(q.field('userId'), args.userId))
      .collect()
    const snippetIds = starrtedSnippets.map((star) => star.snippetId)
    const snippetDetails = await Promise.all(snippetIds.map(id => ctx.db.get(id)))

    const starredLanguges = snippetDetails.filter(Boolean).reduce((acc, curr) => {
      if (curr?.language) {
    ...
    const mostStarredLanguage = Object.entries(starredLanguges).sort(([, a], [, b]) => b - a)[0]?.[0] ?? "N/A"
